...
import type { SqlError } from '../errors/sql-errors.ts';

describe('GenresRepo', async () => {
  const pool = await connectDB();
  const genresRepo = new RepoGenres(pool);
  ...
  describe('Read operations', () => {
    ...
    it('should read a genre by id', async () => {
      ...
      // Example:
      const genre = await genresRepo.readGenresById(1);
      assert(genre);
    ...
    it('should throw an error if genre not found', async () => {
      ...
      // Example:
      try {
        await genresRepo.readGenresById(10);
